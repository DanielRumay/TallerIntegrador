...
public class PromptTemplateService {
    ...
    private String structuredOutput(String tipo, String bloom, String texto, int cantidad, java.util.List<String> preguntasEvitar) {
        ...
        7. Si el tipo es DETECCION_ERRORES, el 'enunciado' debe ser un parrafo fluido que contenga de 2 a 3 errores conceptuales sutiles basados en el texto. 'opciones_o_respuesta' contendra exactamente esas palabras con errores, y 'respuesta_correcta' contendra las correcciones exactas separadas por el caracter '|' en el mismo orden. CRÍTICO DE LONGITUD Y CONCORDANCIA Y GRAMÁTICA: La sustitución directa del error por la respuesta_correcta en el texto DEBE ser gramaticalmente impecable. Por ejemplo, si la frase es 'confiando en la tecnología digital' y la corrección es 'efectos in camera' (masculino plural), se produciría 'confiando en la efectos in camera', lo cual es gramaticalmente incorrecto. Para evitar esto, debes: a) O bien hacer que el error y la corrección tengan exactamente el mismo género y número gramatical (ej. femenino singular 'tecnología digital' -> 'tecnología analógica'). b) O bien incluir el artículo/preposición dentro del error y de la corrección (ej. error: 'la tecnología digital', corrección: 'los efectos in camera'), de manera que al hacer el reemplazo la frase quede impecable: 'confiando en los efectos in camera'. ¡Esto es de suma importancia para que no haya incoherencias gramaticales en la frase resultante!
